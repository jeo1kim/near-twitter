...
import { Message, Post, ContributionTracker, Vector } from "./models"
...
export class DashboardContract {
  ...
  postPublicMessage(message: string): bool {
    ...
    assert(message.length < Message.max_length(), "Message is too long, must be less than " + Post.max_length.toString())
